...
import com.hocs.test.pages.platform.Workdays;
...
public class EndToEndStepDefs extends BasePage {
            ...
            case "COMP":
                switch (stage.toUpperCase()) {
                    case "REGISTRATION (TO SERVICE TRIAGE)":
                        COMPRegistration.moveCaseFromRegistrationToServiceTriage();
                        break;
    ...
    public void getCaseToSpecifiedStage(String stage) {
        ...
        if (!stage.toUpperCase().equals("CLOSED")) {
            dashboard.getAndClaimCurrentCase();
        }
    }

    @And("I move the claim to the {string} stage via Casework Team: {string}")
    public void iGetACaseAtTheStageFromCaseworkTeam(String stage, String teamName) {
        switch (stage.toUpperCase()) {
            case "CASEWORK":
                iCompleteTheStage("Registration (to Triage)");
                dashboard.getAndClaimCurrentCase();
                wcsProgressCase.moveTriageCaseToCaseworkTeam(teamName);
                break;
            case "QA":
                iGetACaseAtTheStageFromCaseworkTeam("Casework", teamName);
                iCompleteTheStage("Casework");
                break;
            case "PAYMENT PRE-OFFER CHECKLIST":
                iGetACaseAtTheStageFromCaseworkTeam("QA", teamName);
                iCompleteTheStage("QA");
                break;
            case "OFFER APPROVAL":
                iGetACaseAtTheStageFromCaseworkTeam("Payment Pre-Offer Checklist", teamName);
                iCompleteTheStage("Payment Pre-offer Checklist");
                break;
            case "SEND OFFER":
                iGetACaseAtTheStageFromCaseworkTeam("Offer Approval", teamName);
                iCompleteTheStage("Offer Approval");
                break;
            case "OFFER ACCEPTANCE":
                iGetACaseAtTheStageFromCaseworkTeam("Send Offer", teamName);
                iCompleteTheStage("Send Offer (To Offer Acceptance)");
                break;
            case "PAYMENT PREPARATION":
                iGetACaseAtTheStageFromCaseworkTeam("Offer Acceptance", teamName);
                iCompleteTheStage("Offer Acceptance (To Payment Preparation)");
                break;
            case "PAYMENT APPROVAL":
                iGetACaseAtTheStageFromCaseworkTeam("Payment Preparation", teamName);
                iCompleteTheStage("Payment Preparation");
                break;
            case "SEND PAYMENT":
                iGetACaseAtTheStageFromCaseworkTeam("Payment Approval", teamName);
                iCompleteTheStage("Payment Approval");
                break;
            case "AWAITING PAYMENT CONFIRMATION":
                iGetACaseAtTheStageFromCaseworkTeam("Send Payment", teamName);
                iCompleteTheStage("Send Payment");
                break;
            case "CLOSED":
                iGetACaseAtTheStageFromCaseworkTeam("Awaiting Payment Confirmation", teamName);
                iCompleteTheStage("Awaiting Payment Confirmation");
                break;
            case "NIL OFFER ACCEPTANCE":
                iGetACaseAtTheStageFromCaseworkTeam("Send Offer", teamName);
            ...
            case "TIER 1":
                ...
                break;
            case "TIER 2":
                iGetACaseAtTheStageFromCaseworkTeam("Tier 1", teamName);
                iCompleteTheStage("Tier 1");
                break;
            default:
                pendingStep(stage + " is not defined within " + getMethodName());
